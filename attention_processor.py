...
class AttentionProcessor:
    ...
    def __init__(
        ...
        # Debug log file path
        self.debug_log_path = Path(config.CACHE_DIR) / "attention_debug.log"
        self._clear_debug_log()
    
    def _clear_debug_log(self):
        """Clear the debug log file at initialization."""
        if self.debug_log_path.exists():
            self.debug_log_path.unlink()
    
    def _log_debug(self, message: str):
        """Write debug message to log file."""
        # Skip all debug messages except attention matrix size information
        if "Attention shape" in message or "attention shape" in message:
    ...
    def get_attention_to_vision_tokens(
        ...
        # Log aggregated attention shape for this token
        self._log_debug(f"[Token {token_position}] Aggregated attention shape: {aggregated_attn.shape}")
        ...
        # Log attention from token shape for this token
        self._log_debug(f"[Token {token_position}] Attention from token shape: {attention_from_token.shape}")
        ...
        if vision_attention_list:
            ...
            # Log vision attention shape for this token
            # self._log_debug(f"[Token {token_position}] Vision attention shape: {vision_attention.shape}")
    ...
    def create_attention_map(
        ...
        # Optional threshold denoise (visualization-only)
        try:
            attention_map = utils.threshold_denoise_attention_map(attention_map)
        except Exception:
            pass

        # Log final attention map shape
        self._log_debug(f"[Attention Map] Final shape: {attention_map.shape}")
